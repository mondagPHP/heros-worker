 ...
 */
class MakeAspectCommand extends AbstractCommand
{
    use MakeTraits;

    protected $name = 'make:aspect';

    protected $description = '快速创建aspect AOP文件';
    ...
    public function run(Input $input = null): void
    {
        ...
        $this->newByStub(function () {
            $stubFile = __DIR__ . '/stubs/aspect.stub';
    ...
    {
        return [
            ['-path', 'require', '以app_path()根目录开始,命名空间与路径对应 eg:app/aspect'],
            ['-class', 'require', 'aspect类名'],
        ];
    }
}
